...
4. **What has no block is counted, never cut.** Naming from contracts is
   ...
   `used by n bodies`. Both at the block's foot, both listed as rows on the
   sheet — that count is exactly the "directly accessed" reading, engraved in
   words because its other end has no block to draw a line to.
...
- **Blocks** share the surface block's anatomy — keyword + visibility in
  ...
  type the row reaches, locator at the foot. No method band. A static quotes
  its declared type under its name.
...
- **Selection** is a URL and a reading: blast radius walks every structural
  ...
  never its box, so a lit kid never dims with its holder. Camera never moves.
...
- Cartouche: workspace name; `n structs · n enums · n statics` (unions when
  present); `n roots · n nested · n standing`; `n body dependences drawn`;
  the four-rung ladder; the diff line, flare counts, insight line. One
  toggle: `references`. No visibility toggle — the legend states why.

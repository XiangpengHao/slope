...
What has no block here is counted, never cut: `named by n signatures` (free fns, method rows, consts, aliases whose declared surface names the type) and `used by n bodies` (references from code with no mark of its own) stand at the block's foot, and the sheet lists both as rows. A reading's recede acts on a block's *own paint* — its frame and text — never on its box, because a receded holder can have a lit block nested inside it. The structural diff rides the shared grammar: letters, flare frames, ghosts standing in their frame, woven rows, event edges.

...
One block per visible struct or enum: hairline frame, then a quotation (revised 2026-08-19, user-confirmed). The header (`pub struct` keyword-blue, the name type-teal at 700, an amber `M` when its file changed), every field as written in declaration order (`multi_crate: bool` included — plain fields were once counted and are now quoted), and an enum's variants as written, payloads and discriminants included (`File(String, String)`), one row each — all colored by token class the way the definition plate colors its source, with the one run that names a held workspace type bold. Counted folds (`+ 4 more fields`, `+ 2 more variants`, `held by 6 types`) count only what overflows the block's eight quoted rows per list, above a locator in soft ink; the folds and locator stay in the chart's monochrome voice. A static wears the gate's 2.5px ink left edge — a root, state no type holds, drawn whatever its visibility — and quotes its declared type whole. The ground is the crate and its top-level module frames (`mod views`), tinted like districts, labeled in rust's own words. Two wire families share the paper: **holds** (structure, at rest — Owns solid, Shares dashed with the wrapper's own word on the line, Borrows dotted with `&`, Dyn dashed with `dyn`, the arrowhead on the holder) and the **reference reading** (the code map's tie grammar at half ink, arrowhead on the user, set by the same `references` toggle). Privacy and the mark budget fold types to counted rows that receive the folded types' edges; a hub past three holders folds its fan-in to `held by n types`, and hovering either end inks a folded wire back in. Outside the quoted rows nothing at this altitude takes a second color.

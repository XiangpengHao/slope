...
### Code pane tokens
Only inside a quotation, and only as token class: the definition plate's code pane, and a data mark's quoted field and variant rows (2026-08-19, user-confirmed — those rows are sliced source, so they carry the pane's palette and it stops at the block's frame). Each clears 4.5:1 on paper, and none sits in the flare's hue, so amber keeps its exclusive meaning even on a pane's own gutter.
...
- **Camera discipline:** the camera moves only for a focus, the first paint of a fresh session, or an explicit refit (`f`). Deselecting never moves the camera, selecting on the data chart never moves it, and a remount is a return: the code and data charts keep their pan and zoom in session state, so coming back from a definition plate (by any path, back button included) restores the camera exactly as the reader left it (2026-08-19). Every move is one 400ms glide (`cubic-bezier(0.65, 0, 0.35, 1)`), 0ms under reduced motion; marks, ring guides, and re-inked edges travel on the same beat so plate and camera move together.
...
One block per visible struct or enum: hairline frame, then a quotation (revised 2026-08-19, user-confirmed). The header (`pub struct` keyword-blue, the name at 700 — type-teal for a struct or union, purple for an enum, so a product type and a sum type read apart at chart zoom where the keyword is small — and the structural diff's letter in git's own alphabet: `A` added since the base, `M` declaration changed, `D` removed; a file-level change alone marks nothing), every field as written in declaration order (`multi_crate: bool` included — plain fields were once counted and are now quoted), and an enum's variants as written, payloads and discriminants included (`File(String, String)`), one row each — all colored by token class the way the definition plate colors its source, with the one run that names a held workspace type bold — a plain type name is from outside the workspace, has no mark, and so has no line drawn to it. Counted folds (`+ 4 more fields`, `+ 2 more variants`, `held by 6 types`) count only what overflows the block's eight quoted rows per list, above a locator in soft ink; the folds and locator stay in the chart's monochrome voice. The row folds are a rest state, not a cut: selecting the block quotes every field and variant it has (revised 2026-08-19, user-confirmed), and the plate grows down over the neighbours that are receding anyway — its resting box is what the edges still land on, so nothing else moves. A static wears the gate's 2.5px ink left edge — a root, state no type holds, drawn whatever its visibility — and quotes its declared type whole, with the workspace type that type reaches bold, as a field row's is: `GlobalSignal<Trail>` bolds `Trail` and draws its line, `GlobalSignal<Option<Viewport>>` bolds nothing, because a dependency's type has no mark to point at. The ground is the crate and its top-level module frames (`mod views`), tinted like districts, labeled in rust's own words. Two wire families share the paper: **holds** (structure, at rest — Owns solid, Shares dashed with the wrapper's own word on the line, Borrows dotted with `&`, Dyn dashed with `dyn`, the arrowhead on the holder) and the **reference reading** (the code map's tie grammar at half ink, arrowhead on the user, set by the same `references` toggle). Privacy and the mark budget fold types to counted rows that receive the folded types' edges; a hub past three holders folds its fan-in to `held by n types`, and hovering either end inks a folded wire back in. Outside the quoted rows nothing at this altitude takes a second color.

The **structural diff** (2026-08-19) is drawn in this same grammar, against the base edition of every changed file (read syntactically, matched by kind and name — the legend states the method). A diff-touched block wears the flare on its own frame and its letter beside its name. A removed type or static is a **ghost**: the same block dashed, rows quoted from the base, locator `path:line (base)`. An added field or variant wears a flare `+`; a dropped one is quoted from the base, struck, seated where it stood — and the resting row window stretches to the last diff row, so diff ink never hides behind a fold. A holding edge the base could not have drawn takes flare with `added` after the wrapper's word; one only the base had is flare-dashed with `removed`. Diff marks and edges never fold, whatever the budget, and while the diff has anything to say the untouched marks rest at a lighter pressure (hover restores). A clean diff draws none of this, and the chart reads as the monochrome engraving the Flare Monopoly promises.
...
At the **data altitude** (`/data`), structs, enums, and statics seat as blocks inside their crate's top-level module frames. Clicking a type selects it (revised 2026-08-19, user-confirmed): the selection is a URL (`/data/type/:..path?item=`) and a reading — the chosen block wears the app's focus ring and opens to every field and variant it quoted a count for, everything a shape change to it could reach (its transitive holders, walked holder-ward) keeps full ink with its wires, folded ones inking back in, what it directly holds keeps ink one hop down, ties touching the selection keep their own, and every other mark and wire recedes to a lighter pressure over the same 400ms beat. Nothing moves and the camera holds still. A selection sheet on the right column names the selection, lists who holds it and what it holds — every row re-centers the selection — states the blast radius in words (`a shape change here reaches 9 more types upstream.`), and carries the one explicit step further: `open its definition →` to the code plate, which stays the only quotation surface. Escape, bare paper, or the selected block clicked again deselects. Hovering a block still inks every edge that touches it, folded ones included; `f` refits (reserving the sheet's column while one is open), and `←` / `→` retrace history as everywhere. The `/code` and `/data` routes share one survey fetch, so stepping between those two rungs never re-runs rust-analyzer.

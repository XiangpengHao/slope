...
The atlas has two altitudes of that one plate: the dependency chart (`/`, crates as stars on rings of hops) and the code chart (`/code`, files as blocks seated inside nested directory frames). The altitude ladder — a `dependencies · code` line in every cartouche, the current rung engraved solid, the other a link — is the only navigation between them. Both altitudes share the plate, the legend discipline, and the URL-per-focus rule; the code altitude adds directory frames, file blocks, reference ties, and the definition plate without adding a second material.
...
- Two altitudes, one grammar: edges and ties draw only for the selection, on both charts.
...
- **Hairline Ink** (#949dab): constellation lines and reference ties, ring guides, file-block and directory frames, the code pane's frame, thin rules inside plates, hover underlines, minimap marks. Never used for body text.
...
The code map's ground layer, under the ties and blocks: one bordered frame per open directory, filled at 2.6% ink so nesting reads as depth without a second material. Nesting means one thing — belongs to. The frame's label band sits on its top border and is placed segment by segment, each measured on its own face: the directory as it is on disk (`▾ views/`, mono 500 at 12px, clicking folds it), then its counts (`12 files · 255 items`, 9px soft ink), then the crate whose sources live under it (`slopify`, 10px soft ink, a link to the crate sheet). A frame with no room for the counts drops them — a truncated name names nothing, and the crate outranks a repeated tally. The root frame carries the workspace name and never folds. Every segment wears a paper halo.
...
Every reference between two territories, summed, drawn at rest — not only for a selection. Quadratic curves bowed toward open paper (16% of length, capped at 52px), width `0.55 + count × 0.13` capped at 2.8px, the arrowhead resting on the **user** — the way change travels. The heaviest dozen carry their count at rest in 9.5px soft-ink mono with a paper halo and draw at full weight; lighter ties draw at half opacity and keep their count folded until the reader hovers either end, which brings that territory's ties up to full ink. A file folded inside a gate still shows its ties, ending at the gate that stands in for it. Ties ink in over 400ms (`edge-ink`, opacity only), none under reduced motion.
...
Selecting a whole **file** gives the same plate with an outline instead of a quotation — a file has no single definition to quote. The locator is the path, the facts line reads `373 lines · 24 items · crate slopify`, and each row is `pub enum DepKind`, its reference count, and its line number.

The two reference columns are grouped by the file the references are written in: a header naming the file with its total (`37 refs`), then rows of keyword + name + count, capped at 3 with `+2 more (5 refs)`. A file's private items lift into one counted row named `private items`. An empty column says `No references.` / `No outgoing references.` Every row re-centers the plate on itself.
...
### Legends
One per altitude, both plates with a `fold` summary in Garamond tracked caps. Content reads in fixed order: the key first (every mark and line the chart can draw, plus the `M` marker, rendered by the same components as the chart), then the gestures, then the survey's own honesty notes in ink-soft (unresolved-name counts, what the survey does not resolve). The code legend carries no glyph key: item kind is a keyword and needs none. When the key runs past the plate's height it scrolls under an **always-visible** thin ink scrollbar (`.legend-scroll`, 6px, ink-line thumb on a 7% ink track): the fold is drawn, never hidden.
...
### Navigation
Navigation is the chart, at two altitudes joined by the cartouche's altitude ladder and by cross-links in the panels (the dep chart's focus panel offers "its files ↓"; the code chart's crate sheet offers "its dependencies ↑").
...
At the **code altitude** (`/code`), files seat as blocks inside their directory's frame. Clicking a file pushes `/code/file/:..path` and opens its definition plate; clicking an item row selects `?item=Type::name` and quotes that item's source; clicking a directory label or gate folds or opens it (a view state, not a URL); a frame's crate name pushes `/code/crate/:name`. Escape steps up one focus level — item → file → whole map — `/` finds a file or item, `f` refits. Every focus on both charts is a URL — there is no navbar or sidebar; the URL, the back button, and the panel's breadcrumb are the trail.

**The Selection's-Ink Rule.** On the dependency chart, edges are drawn only for the current selection; the resting chart is stars on an engraved ground, never a hairball. The code map draws its ties at rest, because coupling between files is the thing that altitude exists to show, but only the heaviest dozen state their counts until a block is hovered.
...
**The Drawn-Fold Rule.** Whatever is elided says so in words and stays reachable: gates write their file counts, blocks write `+ 9 quieter pub · 5 private`, a folded function body writes `+ 42 more lines`, lists chunk with "show all n" and `+2 more (5 refs)`, and the legend scrolls under an always-visible ink scrollbar. Nothing is silently cut.

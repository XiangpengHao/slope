...
typography:
  ...
  far-name-data:
    ...
    lineHeight: 1.05
  far-name-fn:
    # measured per block in views/func/map.rs; 26px ceiling, 11px floor
    fontFamily: "JetBrains Mono, ui-monospace, SFMono-Regular, monospace"
    fontSize: "26px"
    fontWeight: 700
    lineHeight: 1.06
...
- Every focus is a URL (`/dep`, `/dep/crate/:name`, `/dep/ring/:hop`, `/data`, `/data/mark/:..path?item=`, `/data/mod/:..module`, `/fn`, `/fn/mark/:..path?item=`, `/fn/mod/:..module`, `/fn/depth/:band`; `/` redirects to `/dep`, the rung a review starts on); the back button retraces the review trail.
- Square corners everywhere; depth by ruling, not by shadow.
- Three altitudes, one grammar: the dependency chart draws edges only for the selection; the data chart says plain same-module ownership as nesting, draws only what nesting cannot say, and thins its body dependences to the reading its own toggle names; the function chart rests one wire per mark — the shortest way in — and folds every other resolved call to a hover. None of the three ever draws the whole graph at once.
...
**The far edition has its own ramp**, and it is the only place these sizes are legal. Below reading zoom the camera has already scaled the paper down by two thirds or more, so the far edition draws at sizes that are unreadable at 1.0 and correct at 0.3: the keyword above a name at 13px on both charts; a data block's name at 30px (21px in a block under 230px wide) and its diff letter at 24px; a function block's name **measured per block** between an 11px floor and a 26px ceiling — the size at which that identifier fits its own box on one line, written into the block as `--far-name` by `views/func/map.rs` — and its diff letter at 22px; a band caption at 26px, a prism name at 24px, a bundled boundary count at 26px. These sizes are not steps in the reading ramp and never appear on a plate: a plate is drawn at screen scale, and a 26px mono word on one would be a shout.

**The Two-Ramps Rule.** Type on the paper is sized for the zoom that draws it. The reading ramp (8.5–19px) is what a plate and a block at reading zoom may use; the far ramp (11–30px, and the measured `--far-name`) is what the chart may use below 0.45 zoom, and neither borrows from the other. A far name is measured, never guessed: breaking an identifier across two lines and clipping the second (`DataCartouch/e`) is not a smaller name, it is a wrong one.
...
- **Legibility floor:** fit-view never zooms below a floor — dependency chart 0.7 focused / 0.22 overview; data chart 0.22; function chart 0.18 for an explicit refit, but the *opening* view holds 0.34–0.75 and puts the reader on the entry band rather than fitting a sheet of three hundred declarations into dust. The function fit leaves 6% (fit × 0.94), as the data fit does. Past the floor the chart holds readable scale and pans, seating the focal point mid-frame.
- **Camera discipline:** the camera moves only for a focus, the first paint of a fresh session, or an explicit refit (`f`). Deselecting never moves the camera, and a remount is a return: every chart keeps its pan and zoom in session state (the function chart keeps one camera per seating, since a section of types is not a section of modules), so coming back to a chart (by any path, back button included) restores the camera exactly as the reader left it (2026-08-19). Every move is one 400ms glide (`cubic-bezier(0.65, 0, 0.35, 1)`), 0ms under reduced motion; marks, ring guides, and re-inked edges travel on the same beat so plate and camera move together. One amendment (2026-08-21, user decision): selecting a mark the glass cannot show — off the viewport, or below reading zoom — glides the camera to it, because a selection the reader cannot see is not a focus; a selection already legible moves nothing. It began on the data chart the day a row's held type became a link to a block that can stand anywhere on the paper.
...
### The Function Mark (signature, function altitude)
One block per declaration that runs — a function, a method, a trait's own method clause, a `macro_rules!` — drawn as the data mark's dual and in its grammar: a hairline ink-line frame (1px) on paper, 5/7/4px padding, square, holding a quotation rather than a summary. The head is the keyword run in keyword-blue (`pub(crate) fn`), the name at 700 in the palette's fn purple (#6b3fa0), the diff's letter in flare beside it, and the punctuation that **opens the parameter list**; the rows are the parameters, quoted as rust writes them — a method's receiver first and exactly as written (`&mut self`), each parameter name in ink and its type in type-teal, stood in 12px from the edge the way a body is indented; the closing line carries the return. Nothing is reconstructed and nothing is alphabetized: a signature the file does not write is not quoted. Head 16px at 10.5px type, rows and tail 15px at 10px; the block is sized between 152 and 300px wide by its own longest line.
...
- **A mark in a call ring goes dashed.** Where no entry point reaches a declaration, its frame takes the world's own mark for absence. It is not dead code — the survey cannot follow every `dyn` call — and the band's own words say so.
- **The picked block** takes the app-wide focus ring (1.5px ink outline, 2px offset; 5px at 6px offset in the far edition). A reading's pressure acts on the block's paint, never on its box: 0.78 near, 0.32 dim, 0.62 for the whole plate at rest while the diff has something to say, restored on hover.
...
**Two families, and only two.** *Solid* is a **call** — at this altitude a body is the declaration, so calls are the structure the chart is made of. *Dashed and lighter* (`5 3` at 0.5), labelled `answers`, is a **contract**: a trait's own method clause and the methods that answer it, which is what carries reachability across a `dyn` call the survey cannot follow. The arrowhead rests on the dependent in both, as it does at every altitude — the caller of a call, the answering method of a clause. The contract family is drawn first, under the calls, so a promise never covers a run.

**The resting reading is the way in** (the chart's whole disclosure rule). For every mark the plate draws the one call that put it at its depth — the shortest way something that starts reaches it. That is a tree, one wire per mark, and it is the whole of *what runs from where*; drawing all fifteen hundred resolved calls would be the hairball this system forbids one rung up. Every other resolved call stays in the set **folded** and inks back on hover or selection of either end. One exception, and it is structural: a **contract wire never folds**, because it is what makes the tree honest about a `dyn` call.

**The ground is a section.** Full-width bands of call depth run the sheet, captioned at the left margin in the data voice with a paper halo (`entry`, `1 call deep`, `4 calls deep`, `in a call ring`) over a 0.8px hairline rule; the caption rides the readable glass so panning never takes it off-screen, and the caption is the control — hovering underlines it, clicking selects the whole band (`/fn/depth/:band`). Crossing every band are **prisms**, one per frame, boxed in a 0.8px `2 4` dashed hairline and named with the frame's whole path along the top (`views::data`) in 10px mono at 0.06em; the name is the control that selects that boundary. A selected band takes full ink on its caption (700) and a 1.2px rule; a selected prism drops its dash for solid ink. Strangers recede — 0.35 for a band, 0.45 for a prism — and nothing moves.

**Three readings ride the cartouche**: `group` (module / type / file — what a prism is, nested inside the module frame, never replacing it), `calls` (calls / callers / both, read against an anchor — the selection, the hovered block, or on the resting plate the diff, exactly as the data chart's `references` reading is anchored), and the same four-stop `visibility` slider the data chart slides (`pub` → `pub(crate)` → `pub(super)` → `all`, with `n off` at its foot). The census above them is the chart's own: `204 functions · 182 methods`, then the entry-point count and the ring count as **links to their bands**, then the diff line — two count lines and the diff, as the Census-Not-Inventory Rule allows.

**The selection sheet keeps the data sheet's discipline** (288px plate, right column): `Called by`, and under it a fold naming the transitive callers in words (`a rewrite reaches 9 more upstream`); `Calls`, which is what the body runs and the contract it answers; and `Data touched` — every workspace type the signature names or the body uses, each row a step **down to the rung that draws types**. That last heading is the one link between the rungs, and it is why the two charts are duals rather than neighbours. An empty `Called by` reads as the whole verdict in one sentence: `nothing in the workspace calls it.` Rows the chart draws no block for open as quotations on the plate to the sheet's left, the same `peek=` URL the data chart uses.

**A band selection has a sheet of its own**, and it lists rather than counts: the band's caption as its title, one sentence saying what the depth means (`22 declarations nothing in the workspace calls.`), and then every declaration at that depth by name, each a link, each with the file and line it is written on. A band lights its marks and the way in to each, and folds nothing back — a stratum holds sixty marks, and unfolding sixty fan-ins there would be the hairball. A count of sixty resolvable declarations with no way to reach them is the defect this system rejects everywhere else.

**The far edition** at this altitude is the data chart's, sized for a wider block: rows, tail and opening punctuation retire, the keyword stands over the measured name, the entry edge and the focus ring hold near-constant screen width, wire words retire while the wires thicken to 2px — and the **ground takes the heavier pressure**, band rules to 2.4px at full opacity and prism boxes to 2px `6 10`, because the section is the structure at that distance and a 3px wire over a 0.8px rule would read as the hairball the chart refuses. Band captions and prism names grow with it (26px and 24px, halos at 10px and 9px) and are read at full ink: soft ink at 55% is under 3:1 once the camera has scaled a 24px word down to eight.
...
At the **function altitude** (`/fn`), every declaration that runs seats at the crossing of its call depth and its frame, so both readings come off the paper at once. The plate opens on the entry band at a scale its names can carry. Clicking a mark selects it (`/fn/mark/:..path?item=`) and lifts its sheet; clicking a prism's name selects that boundary (`/fn/mod/:..module`); clicking a band's caption selects the whole stratum (`/fn/depth/:band`), and the cartouche's entry-point and call-ring counts are themselves links to their bands. `group` re-seats the paper, and each seating keeps its own camera — a section of types is not a section of modules, and handing a reader the other one's camera would lose their place. Search (`/`) runs over declarations, a row being its keyword, its name, and `src/views/func/map.rs:162`; Escape, bare paper, or the selected block clicked again deselects; `f` refits the whole sheet.

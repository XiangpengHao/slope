...
- **Camera discipline:** the camera moves only for a focus, the first paint of a fresh session, or an explicit refit (`f`). Deselecting never moves the camera, selecting on the surface chart never moves it, and a remount is a return: the code and surface charts keep their pan and zoom in session state, so coming back from a definition plate (by any path, back button included) restores the camera exactly as the reader left it (2026-08-19). Every move is one 400ms glide (`cubic-bezier(0.65, 0, 0.35, 1)`), 0ms under reduced motion; marks, ring guides, and re-inked edges travel on the same beat so plate and camera move together. One amendment (2026-08-21, user decision): on the data chart, selecting a mark the glass cannot show — off the viewport, or below reading zoom — glides the camera to it, because a selection the reader cannot see is not a focus; a selection already legible moves nothing.
...
- Every cartouche carries the altitude ladder (`dependencies · code · surface · data` in 9.5px tracked mono caps): the current rung engraved solid ink with an underline, the others soft-ink links. Under it the cartouche states its own facts in mono — counts, the diff line (`diff main @ 1a2b3c4 → working copy`), and how many files changed.
...
### Legends
One per altitude, each a plate with a `fold` summary in Garamond tracked caps. Content reads in fixed order: the key first (every mark and line the chart can draw, plus the `M` marker, rendered by the same components as the chart), then the gestures, then the survey's own honesty notes in ink-soft (unresolved-name counts, what the survey does not resolve). The code legend carries no glyph key: item kind is a keyword and needs none. The surface legend carries the two inks with drawn samples, the contract plate and the trait block, the static's edge and the one-liners, the two name colors, the quotation rule and the fan-in fold, and the diff's key — then its own honesty notes: the wrapper table, the holes (type parameters, trait bounds, `impl Trait`), what a macro declares that the chart cannot read, and what a folded module takes off the paper.

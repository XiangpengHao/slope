...
The palette had four more classes while the code map quoted whole item bodies — fn/macro name (#6b3fa0), string (#9c2f4a), doc comment (#2f6b45), comment (#5f7060). A declared row says none of those, so they went with the map (2026-08-24).
...
- **Chart Heading** (Garamond 400/600, 11–13px, uppercase, 0.18–0.26em tracking): plate section headings — "Changes (n)", "Used by (n)", "Depends on (n)", "Held by (n)", "Implements (n)", the toggle caption.

    ...
    while let Some(&c) = chars.peek() {
        if c.is_alphanumeric() || c == '_' {
            ...
            match targets.get(&run) {
                ...
                Some(_) => *ambiguous += 1,
                None => {
                    if run == "dyn" && HoldKind::Dyn.rank() > kind.rank() {
                        kind = HoldKind::Dyn;
                        via = "dyn".to_string();
                    }
...
impl CodeGraph {
    /// The crate a base-edition path belonged to: the live file sharing the
    /// most leading path segments wins, so a deleted file lands beside its
    /// neighbors.
    fn krate_for(&self, path: &str) -> String {
    ...
    /// Compute the structural diff and write it onto the graph: per-item
    ...
    pub(super) fn apply_base_diff(
        ...
        let live_pair: HashSet<(u32, u32, bool)> = self
            .holds
            .iter()
        ...
        let mut gone: Vec<HoldEdge> = gone
            ...
            .collect();
        gone.sort_by(|a, b| {
            (a.from, a.to, a.kind.rank(), &a.via, a.from_method).cmp(&(
                b.from,
                b.to,
                b.kind.rank(),
                &b.via,
        ...
        for path in &diff.changed_files {
            ...
            let Some(text) = diff.file_at_base(dir, path) else {
                continue;
            };

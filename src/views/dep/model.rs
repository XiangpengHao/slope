...
impl RadialLayout {
    ...
    pub(crate) fn build(graph: &WorkspaceGraph, cap: u32) -> Self {
        ...
        let mut placed: HashMap<String, StarPlace> = HashMap::new();
        for (&id, &h) in &hops {
            let a = angle[&id];
            let r = h.min(cap);
...
mod tests {
    ...
    fn rings_are_min_distance() {
        ...
        let l = RadialLayout::build(&g, u32::MAX);
    ...
    fn cap_collapses_deep_crates_onto_last_ring() {
        ...
        let capped = RadialLayout::build(&g, 2);
        ...
        let full = RadialLayout::build(&g, u32::MAX);
